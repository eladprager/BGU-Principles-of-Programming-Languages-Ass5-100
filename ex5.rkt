...
(define sqrt2
  (lambda (x init epsilon)
    (car (find-first (sqrt-lzl x init) (lambda (x) (< (cdr x) epsilon))))
  )
)
...
(define collect-all-values-1
 (lambda (list-assoc-lists key)
  (if (empty? list-assoc-lists)
        '()
        (let ((car-list (get-value (car list-assoc-lists) key)))
        (if (eq? car-list 'fail)
            (collect-all-values-1 (cdr list-assoc-lists) key)
            (cons car-list (collect-all-values-1 (cdr list-assoc-lists) key)))))))
...
(define collect-all-values-2
 (lambda (list-assoc-lists key)
  (if (empty? list-assoc-lists)
        '()
        (get-value$ (car list-assoc-lists)
                    key
                    (lambda (x) (cons x (collect-all-values-2 (cdr list-assoc-lists) key)))
                    (lambda () (collect-all-values-2 (cdr list-assoc-lists) key))))))

;;tests
;(take (sqrt-lzl 2 1) 3)
;(find-first (integers-from 1) (lambda (x) (> x 10))) ;--> 11;
;(find-first (cons-lzl 1 (lambda() (cons-lzl 2 (lambda () '())))) (lambda (x) (> x 10))) ;--> 'fail
;(sqrt2 2 1 0.0001) ;→ 1 169/408

;(get-value '((a . 3) (b . 4)) 'b)
;(get-value '((a .3) (b . 4)) 'c)
;(get-value$ '((a . 3) (b . 4)) 'b (lambda (x) (* x x)) (lambda() 'fail))
;(get-value$ '((a .3) (b . 4)) 'c(lambda (x) x) (lambda() 'fail))

;(define l1 '((a . 1) (e . 2)))
;(define l2 '((e . 5) (f . 6)))
;(collect-all-values-1 (list l1 l2) 'e)
;(collect-all-values-1 (list l1 l2) 'k)
;(collect-all-values-2 (list l1 l2) 'e)
;(collect-all-values-2 (list l1 l2) 'k)
